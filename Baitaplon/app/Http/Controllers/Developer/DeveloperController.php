...
//        dd($user->notifications);
    ...
    public function account(Request $request){
        ...
        if ($get_image) {
            // Kiểm tra: Tên ảnh không được rỗng VÀ nó phải chắc chắn là một file
            if ($account->image != '' && is_file($path.$account->image)) {
                unlink($path.$account->image); 
            }
            
            // Xử lý lưu tên file ảnh mới
            $get_name_image = $get_image->getClientOriginalName();
            $name_image = current(explode('.', $get_name_image));
            $new_image = $name_image.rand(0,99).'.'.$get_image->getClientOriginalExtension();
            
            // Di chuyển file ảnh vào thư mục public/img/account
            $get_image->move($path, $new_image);
            
            // Cập nhật tên ảnh mới vào database
            $account->image = $new_image;
            
        } else { // <-- Đã xóa dấu ngoặc thừa ở đây
